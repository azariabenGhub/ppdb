...
<body>
    ...
    <script>
        ...
        async function getTemplateLinks() {
            try {
                ...
                return templates.map(t => `<a href="/api/template-surat/download/${t.id}?token=${token}" target="_blank">Download ${t.nama}</a>`).join(' | ');
            } catch (e) {
    ...
    `;
        ...
        async function loadDaftarUlangSection() {
            const container = document.getElementById('konten-daftar-ulang'); // ← ID yang benar
            try {
                ...
                if (cek.sudah_mengirim) {
                    ...
                    if (cek.status === 'ditolak') {
                        formHtml = await tampilkanFormDaftarUlang();
